...
class Mask56CycleGANModel(BaseModel):
    """
    ...
    def forward(self):
        ...
        ave_thres = [50.41741741741742, 81.48448448448448, 112.01501501501501, 142.18218218218217, 170.93393393393393, 195.8968968968969, 213.71871871871872]
        B_stain = np.where(B_channel_0 > ave_thres[1], 255.0, 0.0)
        
        # B_channel_2 = np.expand_dims(B_norm_image[:, 2, :, :], 1)
        # ave_thres = [60.4034034034034, 90.98198198198199, 120.12512512512512, 148.54554554554554, 176.17717717717719, 200.18618618618618, 218.981981981982]
        # B_stain_cell = np.where(B_channel_2 > ave_thres[1], 255.0, 0.0)

        # B_res = get_and(B_stain_cell, B_stain)
        # B_res = xor(B_stain, B_res)
            #   crushed1 = np.where(channel_choice > ave_thres[6], 0.0, channel_choice)
            #         crushed1 = np.where(crushed1 < ave_thres[4], 0.0, channel_choice)

        B_channel_1 = np.expand_dims(B_norm_image[:, 1, :, :], 1)
        ave_thres = [55.232232232232235, 85.22022022022023, 114.27827827827828, 143.002002002002, 171.25225225225225, 196.03603603603602, 214.7907907907908]
        crushed1 = np.where(B_channel_1 > ave_thres[6], 0.0, B_channel_1)
        B_res = np.where(crushed1 < ave_thres[4], 0.0, B_channel_1)

        self.B_cell_mask = torch.tensor(np.where(B_res > 127, 1.0, -1.0)).cuda()
