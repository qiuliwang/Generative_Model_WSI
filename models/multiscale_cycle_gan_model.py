...
class MultiscaleCycleGANModel(BaseModel):
    """
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:  # define discriminators
            ...
            self.netD_B = networks.define_D(opt.input_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
            self.netD_L_A = networks.define_DL(opt.output_nc, opt.ndf, opt.netD, label,
                                            opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
            self.netD_L_B = networks.define_DL(opt.input_nc, opt.ndf, opt.netD, label,
                                            opt.n_layers_D, opt.norm, opt.init_type, opt.init_gain, self.gpu_ids)
        ...
        if self.isTrain:
            ...
            self.criterionLabel = torch.nn.L1Loss()
    ...
    def set_input(self, input):
        ...
        inputa_label = input['A_label']

        inputb = input['B']
        inputb_mask_cell = input['B_mask_cell']
        inputb_label = input['B_label']
        ...
        self.real_A_label = inputa_label.to(self.device)
        self.real_B_label = inputb_label.to(self.device)
    ...
    def backward_D_label_basic(self, netD, real, fake, label):
    ...
    def backward_D_A(self):
        ...
        # self.label_loss_A = self.backward_D_label_basic(self.netD_L_A, self.real_B, fake_B, self.real_A_label)
        self.loss_D_A = self.backward_D_basic(self.netD_A, self.real_B, fake_B) 

    def backward_D_B(self):
        """Calculate GAN loss for discriminator D_B"""
        fake_A = self.fake_A_pool.query(self.fake_A)
        # self.label_loss_B = self.backward_D_label_basic(self.netD_L_B, self.real_A, fake_A, self.real_B_label)
